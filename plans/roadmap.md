...
| **C-9** | Security hardening: guards for untrusted ingest | ◐ doctrine written, guards partly built | The boundary was designed in with C-3, not retrofitted. **`architecture.md` §Security** now states the doctrine (mirroring ai-life's): the trust ladder per `fragment.source`, least authority (this server is read-only and has no confirm gate — the shell holds it), deliberate outbound, env-only secrets, and a checklist for new sources. The load-bearing invariant — the analyzer prompt is built from **signatures only**, so comments and literals never reach the model — is now a test, not a comment. What remains is the *hardening*: rate/size limits on ingest, a review pass over the `openai:`/`anthropic:` egress, and whatever a real hostile corpus teaches |
